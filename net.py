...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        """向模块添加持久缓冲区。
        这通常用于注册不应被视为模型参数的缓冲区。例如，BatchNorm的“running_mean”不是参数，而是持久状态的一部分。
        缓冲区可以使用给定的名称作为属性访问。   """
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        assert 0 <= self.shift_size < self.window_size, "shift_size 必须在 0-window_size"
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,out_chans=1,
                 embed_dim=360, depths=[6, 6], num_heads=[6, 6],
        ...
        #its error but in order to load the trained model ,we can only keep
        for i_layer in range(self.num_layers):
            # 实例化 RSTB
            layer = RSTB(dim=embed_dim,
                         input_resolution=(patches_resolution[0],
        ...
        if resi_connection == '1conv':  # 1层卷积
            ...
            self.conv_after_body2 = nn.Sequential(nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1),  # 降维
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim // 4, 1, 1, 0),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 nn.Conv2d(embed_dim // 4, embed_dim, 3, 1, 1))
    ...
    def forward(self, x):
        ...
        return self.lr(x[:, :, :H * self.upscale, :W * self.upscale])
