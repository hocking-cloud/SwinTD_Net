...
inputPathTrain_j = './dataset/gamma'  # 输入的训练文件j
targetPathTrain = './dataset/traintarge'  # 输入训练集的无雾图像
...
if len(device_ids) > 1:
    lowlight = nn.DataParallel(swinIR2, device_ids=device_ids)
optimizer = torch.optim.Adam([
            {'params': lowlight.parameters(),"lr" : 2e-5},
        ])
...
for epoch in range(opt.EPOCH):
    ...
    torch.save(lowlight.state_dict(), './lowlight.pth')
